...
这四种像素格式由Intel IPU3驱动程序使用，它们是带有10位样本的raw sRGB/Bayer格式。每25个像素打包到32字节中，在最后一个字节中留有6位最高有效位填充。格式为小端模式。在其他方面，此格式类似于 :ref:`V4L2-PIX-FMT-SRGGB10`。
以下是V4L2_PIX_FMT_IPU3_SBGGR10格式的小图像示例：
**字节顺序：**
每个单元格是一个字节。
