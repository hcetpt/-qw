...
是否允许，如果允许则发送：
    ...
    COOKIE ECHO ------------------------------------------>
                                                  sctp_sf_do_5_1D_ce()
                                             响应一个 COOKIE ECHO 数据块
确认 cookie 并创建一个
                                             永久关联
调用 `security_sctp_assoc_request()` 来
                                             对 INIT 数据块响应执行相同操作
<------------------------------------------- COOKIE ACK
          |                                               |
    sctp_sf_do_5_1E_ca                                    |
 ...
 设置对等体标签。                                         |
          |                                               |
          |                               如果是 SCTP_SOCKET_TCP 或剥离的套接字，
          |                               则调用 `security_sctp_sk_clone()` 克隆新套接字
|                                               |
...
上面的 `SCTP LSM 支持`_ 章节描述了以下 SCTP 安全钩子，并在下面扩展了 SELinux 的具体内容：
...
`security_sctp_assoc_request()`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
将关联 INIT 数据包的 `@asoc` 和 `@chunk->skb` 传递给安全模块。成功返回 0，失败返回错误
::
    @asoc - 指向 SCTP 关联结构的指针
    @skb - 指向关联数据包 skbuff 的指针

安全模块执行以下操作：
    如果这是 `@asoc->base.sk` 上的首次关联，则将对等体 sid 设置为 `@skb` 中的值。这将确保只有唯一的对等体 sid 分配给可能支持多个关联的 `@asoc->base.sk`
    ELSE 验证 `@asoc->base.sk peer_sid` 是否与 `@skb peer sid` 匹配，以确定是否允许或拒绝该关联
    将 `sctp "@asoc sid"` 设置为套接字的 sid（从 `asoc->base.sk` 获取），并从 `@skb peer sid` 中获取 MLS 部分。这将被 SCTP TCP 风格的套接字和剥离连接使用，因为它们会导致生成新的套接字。

如果配置了 IP 安全选项（CIPSO/CALIPSO），则在套接字上设置 IP 选项。

`security_sctp_bind_connect()` 函数
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
根据 `@optname` 检查 IPv4/IPv6 地址所需的权限如下：
...
`SCTP LSM Support`_ 提供了 `@optname` 条目的摘要，并描述了当启用动态地址重新配置时 ASCONF 数据块的处理过程。

`security_sctp_sk_clone()` 函数
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
每当通过 `accept`(2) 创建新套接字（即 TCP 风格的套接字）或当套接字被“剥离”时（例如用户空间调用 `sctp_peeloff`(3)）。`security_sctp_sk_clone()` 将新套接字的 sid 和对等 sid 分别设置为 `@asoc sid` 和 `@asoc peer sid` 中的值。
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
当接收到 COOKIE ACK 时调用，设置连接的对等 sid 为 `@skb` 中的值。

- `@asoc`：指向 SCTP 关联结构
- `@skb`：指向 COOKIE ACK 数据包的 skbuff

策略声明
-----------------
以下类和权限用于支持 SCTP，在内核中可用：
...
SELinux SCTP 支持增加了 `name_connect` 权限，用于连接到特定端口类型，并增加了 `association` 权限，该权限在下面的部分中解释。
