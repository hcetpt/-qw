...
**rtla osnoise hist** 工具将所有 **osnoise:sample_threshold** 的发生收集到一个直方图中，并以用户友好的方式展示结果。该工具还允许对 *osnoise* 追踪器及其输出的收集进行多种配置。
选项
======
...
示例
======
在下面的例子中，*osnoise* 追踪器线程被设置为使用实时优先级 *FIFO:1* 在 CPU *0-11* 上运行，每个周期（默认为 *1s*）运行 *900ms*。减少运行时间的原因是避免让 **rtla** 工具出现饥饿问题。工具也被设置为运行 *一分钟*。输出直方图被设置为按 *10us* 和 *25* 条目分组 ::
