...
如果系统达到停止追踪条件，则打印自动分析结果。此选项
        有助于减少 rtla timerlat 的 CPU 使用率，在不需要收集统计信息开销的情况下启用调试。
示例
=======

在下面的例子中，timerlat 追踪器被分配到 *1-23* 号 CPU 上，并处于自动追踪模式，指示追踪器在发现 *40 us* 或更高的延迟时停止:
  ...
  系统已退出空闲延迟！
    从空闲状态下的最大中断定时器延迟: 17.48 us 在 cpu 4 上
  将追踪保存到 timerlat_trace.txt

在此案例中，主要因素是处理 **timerlat** 唤醒的 *IRQ 处理程序* 所遭受的延迟：*65.52%*。这可能是由于当前线程屏蔽了中断造成的，这可以在阻塞线程堆栈跟踪中看到：当前线程 (*objtool:49256*) 通过 mem cgroup 内的 *raw 旋锁* 操作禁用了中断，同时在 btrfs 文件系统中执行写 syscall。
原始追踪已保存在文件 **timerlat_trace.txt** 中以供进一步分析。
请注意，**rtla timerlat** 被调度而没有改变 *timerlat* 追踪器线程的优先级。通常来说这是不必要的，因为这些线程默认具有 *FIFO:95* 的优先级，这是实时内核开发者用于分析调度延迟的常见优先级。
参考
------
