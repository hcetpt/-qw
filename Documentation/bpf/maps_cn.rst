...
成功时返回进程本地文件描述符，失败时返回负数错误。通过调用 `close(fd)` 可以删除地图。持有打开文件描述符的地图会在进程退出时自动删除。

.. note:: `map_name` 的有效字符包括 `A-Z`、`a-z`、`0-9`、`'_'` 和 `'.'`

**BPF_MAP_LOOKUP_ELEM**

使用 `attr->map_fd`、`attr->key` 和 `attr->value` 在给定地图中查找键。成功时返回零，并将找到的元素存入 `attr->value`；失败时返回负数错误。

**BPF_MAP_UPDATE_ELEM**

使用 `attr->map_fd`、`attr->key` 和 `attr->value` 在给定地图中创建或更新键值对。成功时返回零，失败时返回负数错误。

**BPF_MAP_DELETE_ELEM**

使用 `attr->map_fd` 和 `attr->key` 在给定地图中查找并删除元素。成功时返回零，失败时返回负数错误。
