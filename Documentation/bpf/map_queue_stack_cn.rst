...
`BPF_MAP_TYPE_QUEUE` 提供先进先出（FIFO）存储，而 `BPF_MAP_TYPE_STACK` 提供后进先出（LIFO）存储以供 BPF 程序使用。这些映射支持预览、弹出和推送操作，这些操作通过相应的辅助函数暴露给 BPF 程序。这些操作以如下方式通过现有的 `bpf` 系统调用暴露给用户空间应用：
...
以下代码片段展示了如何使用 libbpf 的低级 API 从用户空间创建一个队列：
