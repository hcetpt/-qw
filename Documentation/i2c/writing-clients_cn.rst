...
在某些情况下，直接调用 I2C 函数会更容易，但许多芯片都有一种寄存器值的概念，可以很容易地封装起来。
以下函数是简单的示例，不应直接复制：
...
使用这种绑定模型的 I2C 设备驱动程序与其他任何 Linux 驱动程序一样工作：它们提供一个 probe() 方法来绑定到这些设备，以及一个 remove() 方法来解除绑定：
```c
static int foo_probe(struct i2c_client *client);
static void foo_remove(struct i2c_client *client);
```
请注意，i2c_driver 不创建这些客户端句柄。该句柄可以在 foo_probe() 中使用。如果 foo_probe() 报告成功（零而不是负状态码），则它可以保存该句柄并在 foo_remove() 返回之前使用它。这种绑定模型被大多数 Linux 驱动程序采用。

当 i2c_driver 中 id_table 的 name 字段中的条目与设备名称匹配时，会调用 probe 函数。如果 probe 函数需要该条目，它可以使用以下方式检索它：
```c
...
如果您确切知道某个 I2C 设备已连接到给定的 I2C 总线，您可以通过简单地用设备地址和驱动程序名称填充 i2c_board_info 结构并调用 i2c_new_client_device() 来实例化该设备。这将创建设备，然后驱动核心会找到合适的驱动程序并调用其 probe() 方法。
如果驱动程序支持不同的设备类型，您可以使用 type 字段指定所需的类型。如果需要，您还可以指定 IRQ 和平台数据。
有时，您知道某个设备已连接到给定的 I2C 总线，但您不知道它的确切地址。这种情况发生在电视适配器上，其中相同的驱动程序支持几十种略有不同的型号，而 I2c 设备地址从一个型号到下一个型号都会改变。在这种情况下，您可以使用 i2c_new_scanned_device() 变体，它类似于 i2c_new_client_device()，除了它接受一个额外的可能的 I2C 地址列表来探测。在列表中第一个响应的地址处创建设备。如果您期望在地址范围内存在多个设备，只需多次调用 i2c_new_scanned_device() 即可。
对 i2c_new_client_device() 或 i2c_new_scanned_device() 的调用通常发生在 I2C 总线驱动程序中。您可能希望保存返回的 i2c_client 引用以供后续使用。
...
通过`i2c_new_client_device()`或`i2c_new_scanned_device()`创建的每个I2C设备都可以通过调用`i2c_unregister_device()`进行注销。如果你不显式调用它，在底层I2C总线本身被移除之前，它将自动被调用，因为设备不能在其父设备之后生存，这是设备驱动模型的基本规则。

### 初始化驱动程序

当内核启动时，或者当你的foo驱动模块被插入时，你需要做一些初始化工作。幸运的是，通常只需要注册驱动模块就足够了：
...
可以使用`module_i2c_driver()`宏来简化上面的代码：
```c
module_i2c_driver(foo_driver);
```

请注意，一些函数被标记为`__init`。这些函数可以在内核启动（或模块加载）完成后被移除。同样地，被标记为`__exit`的函数在代码构建到内核时会被编译器丢弃，因为它们永远不会被调用。
...
MODULE_DESCRIPTION("Barf Inc. Foo I2C 设备驱动程序");
...
如果你的I2C设备在系统进入低功耗状态时需要特殊处理——比如将收发器置于低功耗模式，或者激活系统唤醒机制——可以通过实现驱动程序的`dev_pm_ops`中的适当回调（如suspend和resume）来完成。
这些是标准的驱动程序模型调用，它们的工作方式与其他任何驱动程序栈相同。这些调用可以睡眠，并且可以使用I2C消息传递给正在挂起或恢复的设备（因为在发出这些调用时，其父I2C适配器处于活动状态，并且IRQ仍然启用）。

### 系统关机

如果你的I2C设备在系统关闭或重启（包括kexec）时需要特殊处理——比如关闭某些功能——可以使用一个`shutdown()`方法。
再次强调，这是一个标准的驱动程序模型调用，其工作方式与其他任何驱动程序栈相同：这些调用可以睡眠，并且可以使用I2C消息传递。
命令功能
================
...
这些函数从/向一个客户端读取和写入一些字节。客户端已经包含了I2C地址，所以你不需要包含它。第二个参数包含要读取/写入的字节，第三个参数表示要读取/写入的字节数（必须小于缓冲区的长度，同时应该小于64K，因为`msg.len`是u16类型）。返回值为实际读取/写入的字节数。
```
