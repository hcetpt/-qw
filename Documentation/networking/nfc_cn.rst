...
近场通信 (NFC) 子系统用于标准化 NFC 设备驱动程序的开发，并创建一个统一的用户空间接口。本文档涵盖了架构概览、设备驱动程序接口描述以及用户空间接口描述。
...
为了与一个目标建立连接，用户必须创建一个NFC_SOCKPROTO_RAW类型的套接字，并用正确填充的sockaddr_nfc结构调用'connect'系统调用。所有信息都来自NFC_EVENT_TARGETS_FOUND网络事件。由于一个目标可以支持多个NFC协议，因此用户必须告知要使用的具体协议。
